...
/*   Updated: 2025/02/20 16:02:59 by tmidik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (data->map->map[y - 1][x] != '1')
	{
		...
		if (x == data->entity.exit_x && (y - 1) == data->entity.exit_y)
		{
			if (check_conditions(data))
				return;
			mlx_put_image_to_window(data->mlx, data->win, data->exit_ship, x * 32, (y - 1) * 32);
		}
		else
			mlx_put_image_to_window(data->mlx, data->win, data->ship_up, x * 32, (y - 1) * 32);
...
void	move_down(t_data *data)
{
	...
	if (data->map->map[y + 1][x] != '1')
	{
		...
		if (x == data->entity.exit_x && (y + 1) == data->entity.exit_y)
		{
			...
			mlx_put_image_to_window(data->mlx, data->win, data->ship_down, x * 32, (y + 1) * 32);
		data->entity.player_y++;
	}
}
...
void	move_right(t_data *data)
{
	...
	if (data->map->map[y][x + 1] != '1')
	{
		...
		if ((x + 1) == data->entity.exit_x && y == data->entity.exit_y)
		{
			if (check_conditions(data))
				return;
			mlx_put_image_to_window(data->mlx, data->win, data->exit_ship, (x + 1) * 32, y * 32);
		}
		else
		
			mlx_put_image_to_window(data->mlx, data->win, data->ship_right, (x + 1) * 32, y * 32);
...
void	move_left(t_data *data)
{
	...
	if (data->map->map[y][x - 1] != '1')
	{
		...
		if ((x - 1) == data->entity.exit_x && y == data->entity.exit_y)
		{
			...
			mlx_put_image_to_window(data->mlx, data->win, data->ship_left, (x - 1) * 32, y * 32);
		data->entity.player_x--;
	}
}
